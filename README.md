...
Three job boards driven by one pipeline. Static pages, no build step, no
...
| Cody · Data Entry | `cody.html` | `profiles/cody.json` | Remote data entry, admin and back-office work that doesn't require prior experience — with pay |
...
Cody's board carries a pay figure on every role, and the two kinds are never
styled alike:

- **Posted** — what the employer published. Ashby returns it in the list
  payload; Greenhouse keeps it one endpoint deeper, so that request is only
  spent on postings that already matched. Lever and SmartRecruiters supply it
  when set. Rendered solid.
- **Estimated** — a US market band for the role family from
  `payEstimate.titleBands`, adjusted by level. Rendered muted, prefixed `~`,
  and says so on hover.

Free text is parsed conservatively: only an explicit `$A – $B` range whose
endpoints agree on magnitude counts. Scanning descriptions for any dollar
amount produced "$1.30/yr" and "$425,000,000" from funding and discount copy,
so anything ambiguous falls through to a labelled estimate instead. Interval is
taken from the stated one, or from magnitude when the ATS omits it — nobody is
paid $37 a year, and two hourly rates once shipped labelled as salaries.
